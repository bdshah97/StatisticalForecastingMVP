...
interface ChatAgentProps {
  ...
  skuMapping?: Record<string, string>; // Maps SKU-001 -> real_sku_name
}

const ChatAgent: React.FC<ChatAgentProps> = ({ provider, audience, context, hasRunAnalysis, skuMapping = {} }) => {
  ...
  // De-anonymize SKU references in AI response
  const deanonymizeText = (text: string): string => {
    let result = text;
  ...
  const handleSend = async () => {
    ...
    const deanonymized = deanonymizeText(response || "I'm sorry, I couldn't generate a response.");
    
    setMessages([...newMessages, { role: 'assistant', content: deanonymized }]);
    setIsLoading(false);
  };
