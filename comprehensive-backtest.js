...
function croston(values, horizon) {
  let n = values.length;
  let alpha = 0.1;
  let z = 0, p = 0;
  let lastNonZero = -1;
  ...
  let forecastArr = Array(horizon).fill(Math.max(0, Math.round(crostonForecast)));
  return forecastArr;
}
...
function calcMetrics(forecast, actual) {
  ...
  // Compute per-pair MAPE: average of |A-F|/A across pairs, skipping A===0
  let sumPct = 0;
  let count = 0;
...
console.log('Month       | Actual | HW Add | HW Mul | Linear | Prophet(Ref) | Prophet(App) | ARIMA(Ref) | ARIMA(App) | Croston\'s');
console.log('------------|--------|--------|--------|--------|-------------|-------------|-----------|-----------|----------');
...
for (let i = 0; i < 6; i++) {
  ...
  row += `${String(forecasts["Croston's"][i]).padStart(8)}`;
  console.log(row);
}
...
// Sort by MAPE (include Croston's)
