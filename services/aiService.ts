...
/**
 * Anonymize data by replacing real SKU names with anonymous IDs
 */
export const anonymizeData = (text: string, skuMapping: Record<string, string>): string => {
  let result = text;
  // Replace real SKU names with anonymous IDs
  Object.entries(skuMapping).forEach(([anonId, realSku]) => {
    const regex = new RegExp(realSku, 'g');
...
/**
 * De-anonymize data by replacing anonymous IDs back to real SKU names
 */
export const deanonymizeData = (text: string, skuMapping: Record<string, string>): string => {
  let result = text;
  // Replace anonymous IDs back to real SKU names
  Object.entries(skuMapping).forEach(([anonId, realSku]) => {
    const regex = new RegExp(anonId, 'g');
    result = result.replace(regex, `${anonId} (${realSku})`);
  });
  return result;
};
...
export const getNarrativeSummary = async (
  ...
  try {
    ...
    // Anonymize SKU names before sending to AI
    let skuList = skus.slice(0, 5);
    if (skuMapping) {
      skuList = skuList.map(sku => {
        // Find the anonymous ID for this real SKU
        const anonId = Object.entries(skuMapping).find(([_, realSku]) => realSku === sku)?.[0];
        return anonId || sku;
      });
    }
    
    const skuPrompt = isNonExec 
      ? `You MUST include a mention of 2 specific SKUs from this list [${skuList.join(', ')}] as high-priority items to monitor.`
...
export const getChatResponse = async (
  ...
  try {
    const systemInstruction = `You are an expert supply chain AI analyst for SSA & Company specializing in demand forecasting and inventory optimization.

CRITICAL INSTRUCTIONS:
1. You have access to real portfolio data provided in the Context Data section below
2. When answering questions about SKUs, ALWAYS cite specific SKU names from the provided portfolio
3. Do NOT use placeholder SKU names like "1234" or "SKU-X" - these are INCORRECT
4. Reference ONLY the actual SKU names mentioned in the Context Data
5. If asked about top SKUs, mention actual SKU names from the "TOP PRIORITY SKUs" list
6. Be specific: use actual SKU identifiers from the portfolio, not generic examples
7. Provide data-driven insights using metrics from the provided context
8. For ${audience}, focus on relevant KPIs and business impact
...
Remember: Always cite REAL SKU names from the provided data. Never use placeholder SKU codes.`;
