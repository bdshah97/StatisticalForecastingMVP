...
// Per-pair MAPE (average of |A-F|/A), skipping zero actuals
const mape_app = (() => {
  let sumPct = 0, count = 0;
  for (let i = 0; i < appAbsErrors.length; i++) {
    const a = actual[i];
    if (a === 0) continue;
    sumPct += (appAbsErrors[i] / a) * 100;
    count++;
  }
...
const mape_ref = (() => {
  let sumPct = 0, count = 0;
  for (let i = 0; i < refAbsErrors.length; i++) {
    const a = actual[i];
    if (a === 0) continue;
    sumPct += (refAbsErrors[i] / a) * 100;
    count++;
  }
  return count > 0 ? (sumPct / count) : 0;
})();
